...
// //     selectIntegrator(integrators: Integrators): (action: AnyAction, inputs: SideInputsType) =>
// //         Promise<util.Result<OutputsType, AnyError>>
...
// //     [K in CollectionId]: (action: AnyAction, inputs: SideInputs[K]) =>
// //         Promise<util.Result<Outputs[K], AnyError>>
// // }
...
// //     ts: Tables,
// //     integrators: Integrators, action: AnyAction): Promise<[string, SavedAction, AnyError | null]> {
...
//             const actionTable = openAll(db)['ACTIONS'];
//             const ts = openAll(db);

//             const [actionId, savedAction, maybeError] =
//                 await livePrimary(db, this.integrators, action);
...
// // export function openActions(db: db.Database): db.Table<SavedAction> {
// //     return db.open({
// //         schema: ['actions'],
